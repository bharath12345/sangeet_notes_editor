...
object KeyHandler:
  ...
  def handleSwarKey(editor: CompositionEditor, key: Char, shiftDown: Boolean): (CompositionEditor, String) =
  ...
  def handleDualSwar(editor: CompositionEditor, key: Char, shiftDown: Boolean): (CompositionEditor, String) =
    val lowerKey = key.toLower
    swarKeys.get(lowerKey) match
      case Some(note) =>
        val variant = resolveVariant(note, shiftDown)
  ...
  def handleSwarGroup(editor: CompositionEditor, notes: List[(Note, Variant, Octave)]): (CompositionEditor, String) =
  ...
  def handleOctaveKey(editor: CompositionEditor, keyName: String): (CompositionEditor, String) =
    keyName match
      case "PERIOD" =>
        (editor.copy(cursor = editor.cursor.withOctave(Octave.Mandra)), "◆ Next note in Mandra saptak (lower octave)")
      case "QUOTE" =>
        (editor.copy(cursor = editor.cursor.withOctave(Octave.Taar)), "◆ Next note in Taar saptak (upper octave)")
      case "BACKTICK" =>
        (editor.copy(cursor = editor.cursor.withOctave(Octave.Madhya)), "◆ Back to Madhya saptak (default octave)")
  ...
  /** Handle a note ornament. Returns (editor, message, nextMode). nextMode is Some if another note input is needed
    * (e.g., Meend second note).
    */
  def handleNoteOrnament(
      editor: CompositionEditor,
      ornamentNote: Char,
      shiftDown: Boolean,
      mode: OrnamentMode
  ): (CompositionEditor, String, Option[OrnamentMode]) =
      ...
      case Some(note) =>
        ...
        mode match
          case OrnamentMode.MeendStart(dir) =>
            ...
            (
              editor,
              s"◆ Meend $dirName start: ${note} — now type the end note",
              Some(OrnamentMode.MeendEnd(noteRef, dir))
            )
          ...
          case OrnamentMode.MurkiCollect(collected) =>
            ...
            (
              editor,
              s"◆ Murki: ${updated.size} notes — type more or press Enter to finish",
          ...
          case OrnamentMode.ZamzamaCollect(collected) =>
            ...
            (
              editor,
              s"◆ Zamzama: ${updated.size} notes — type more or press Enter to finish",
