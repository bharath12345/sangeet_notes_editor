...
class CompositionEditorEdgeCaseSpec extends AnyFlatSpec with Matchers:
  ...
  "setStrokeAt" should "set stroke at cursor position" in {
    ...
    result shouldBe defined
    val swar = result.get.currentSection.events.head.asInstanceOf[Event.Swar]
    swar.stroke shouldBe Some(Stroke.Ra)
  }
  ...
  "swarsAtBeat" should "count swar events at beat position" in {
    val ed = addNote(addNote(mkEditor, Note.Sa, 0, 0), Note.Re, 0, 0)
    ed.swarsAtBeat(0, 0) shouldBe 2
    ed.swarsAtBeat(0, 1) shouldBe 0
  }
